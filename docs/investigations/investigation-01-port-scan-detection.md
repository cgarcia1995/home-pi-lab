...
It only fires if one source sends 20+ SYN packets in 10 seconds. That threshold is how I separate a real scan from normal traffic, a webpage is a few SYNs, a port sweep is multiple fast SYNS.
...
The scan was reconnaissance, checking on every port to see what's open, the usual first move before an attack. It maps to **MITRE ATT&CK T1046 (Network Service Discovery)**.
